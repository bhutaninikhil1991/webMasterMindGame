...
<div id="page">
    <div id="header" class="rounded shadow">
        <div id="heading">MASTERMIND</div>
    </div>
        ...
        <div id="board" class="shadow ${sessionScope.boardWidthClass}">
            ...
            <div class="userarea">
                ...
                <div class="userarearight">
                    <form method="post" name="currentRowForm" id="currentRowForm" action="/mastermind">
                        ${sessionScope.hiddenPegFields}
                        <input type="hidden" name="buttonPushed"/>
                        <input type="hidden" name="defaultHasBeenLoaded" id="defaultHasBeenLoaded"
                               value="${sessionScope.defaultHasBeenLoaded}"/>
                        <input type="submit" value="Check" id="checkResult" name="checkResult"
                               class="fancybutton"/><br/>
                        <input type="submit" value="New Game" id="newGame" name="newGame" class="fancybutton"/><br/>

                        <label>Row Size</label><br/>
                        <select id="codeSize" name="codeSize">${sessionScope.codeSizeDisplay}</select><br/>
                        <label>Allow Duplicates</label><br/>
                        <input type="checkbox" name="allowDuplicates" id="allowDuplicates" checked>
                    </form>
                </div>
            </div>
...
<div id="gameover-dialog-modal" title="Game Over">${sessionScope.gameOverTextDisplay}<br/>
    <input type="button" id="restartGame" name="restartGame" value="New Game" class="fancybutton"></div>
...
<script type="text/javascript">
    ...
    $("#checkResult").click(function () {
        ...
        $('.currentRow > div > div > div:last-child').each(function (index, value) {
            ...
            var match = matchColors.exec(bgColor);
            if (match !== null) {
                currentRowData[index] = match[0];
            }
        });
